...
function addMarker(place)
{
    ...
    for (var key in place) {
        if (key == "latitude") {
            myLatLng.lat= place[key];
        } else if (key == "longitude") {
            myLatLng.lng = place[key];
    ...
    marker.addListener("click", function() {
        $.getJSON(Flask.url_for("articles"), {geo: place.postal_code}, function(articles) {
            var contentString =
            '<div>'+
                '<h1 id="label"><center>' + label + '</center></h1>'
            var list = '<ul>';
            ...
            if (articles.length <= 0) {
                ...
                for (var article in articles) {
                    var article =
                    '<ul>' +
                        '<a href="'+ articles[article].link +'">' + articles[article].title + '</a>' +
                    '</ul>'
                    list += article;
                }
                contentString = contentString + list + '</ul>' + '</div>';
            }
